---
name: refactor-docs
description: Refactor an existing docs page with source-audited preservation, restructuring, and verification.
license: MIT
metadata:
  adapted_from: "https://github.com/openclaw/openclaw/.agents/skills/openclaw-refactor-docs"
---
...
Use this skill when the user gives a target docs page and asks to rewrite,
refactor, reorganize, split, shorten, or improve it.

This skill builds on `technical-documentation` for style, structure, principles,
and review standards. It adds the rewrite workflow needed to avoid losing
...
- A target docs page path, such as `docs/operational-storage.md`.
...
Before editing, decide the intended page type (overview, quickstart, topic,
guide, reference, troubleshooting, governance).
...
- Config fields, flags, commands, env vars, defaults, enums, nullable values,
  and constraints.
- Precedence rules, fallback behavior, caps, limits, timeouts, lifecycle states,
...
- Public schema, generated config docs, or exported types for config fields.
- CLI implementation, help text, and command tests for commands and flags.
- Runtime source and tests for lifecycle, permission, fallback, timeout, and
  provider behavior.
- Protocol docs, SDK facades, and contract tests for APIs.
- ADRs and decision records for ownership and fail-closed policy.
- Existing docs only as secondary evidence unless the target is purely
  conceptual.

If a page promises a reference, compare its tables against schema, CLI help,
exported types, or tests. Missing public fields, defaults, precedence rules,
caps, or side effects are correctness bugs.
...
- New page: choose the page type, path, title, and purpose.
...
- Link integrity for paths you changed (manual or repo docs/link scripts if any).
- `git diff --check -- <touched-files>` when in a git worktree.
- Project formatting/lint for Markdown if configured.
- Run commands and examples from the page whenever feasible.
- For Rust-adjacent claims, use `verify-change` only when code or tests also
  moved; pure prose needs source read-back, not a full cargo suite.

If you cannot verify a behavior-sensitive claim, either remove the claim, mark
the uncertainty in the report, or ask for the missing source.
