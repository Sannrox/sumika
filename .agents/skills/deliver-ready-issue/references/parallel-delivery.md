...
several machines. `AGENTS.md` ("Parallel delivery lanes") owns the policy: lane definition,
GitHub claims, the three-lane limit, collision surfaces, isolation, and
serialization. This reference owns the lead procedure.
...
3. Reject candidate pairs that touch the same collision surface:
   `crates/sumika-protocol`, the PTY supervisor, attach exclusivity / steal,
   or the Unix socket path and same-uid accept. Run such Issues in sequence.
...
- publish only to the claim branch: scripts/gh-verified-push.sh --branch
  <type>/<issue> --sync-local when that script exists, otherwise
  git push -u origin HEAD; open the Pull Request as a draft with
...
1. Each lane publishes to its claim branch from its own worktree under
   Publish authority, after `git fetch --prune origin` and
   `git merge-base --is-ancestor origin/main HEAD` pass. Use
   `scripts/gh-verified-push.sh --branch <type>/<issue> --sync-local` when
   present, otherwise `git push -u origin HEAD`. Publishing may happen
