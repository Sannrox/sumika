...
`sumika` (棲みか, habitat) is a vim-shaped session switcher for CLI agents. Ghostty is the window. Sumika owns the processes.
...
An operator runs `sumika` in one Ghostty window. Five sessions live in the daemon. Attach is a raw PTY. Detach leaves the child. A documented chord always gets back to the picker. Hooks tell sumika `idle` or `blocked`; anything else is `unknown`.
...
4. Ghostty never owns the process.
...
Phase 0 proves one PTY survives detach. Phase 1 is the daily driver for five agents, including Kiro, with a picker and launchd. Phase 2 is hook attention. Phase 3 is Ghostty-as-frame. Phase 4 is last-frame snapshot. Pane splits are not on the path.
